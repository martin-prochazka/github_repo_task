...
import {mockedGitHubRepositories, mockedRepositoryModels} from 'repositories/components/__mocks__/repositoryMocks'
import {fetchRepositories, initialState, repositoryAdapter, repositorySlice} from 'repositories/slice'
...
describe('repositories/slice', () => {
    ...
    it('should fetch mocked repositories', async () => {
        expect(getState().repos.repositories).toEqual(repositoryAdapter.getInitialState())

        await dispatch(fetchRepositories())

        const expectedState = createNextState(getState().repos.repositories, (repositories) => {
